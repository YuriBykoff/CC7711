...
# Converter colunas numéricas para o tipo float (se existirem)
...
plt.savefig('matriz_confusao_bank.png', dpi=300)
plt.show()
...
plt.title('Árvore de Decisão - Dados Bancários', fontsize=20)
plt.savefig('arvore_decisao_bank.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.title('Importância das Features - Dados Bancários', fontsize=16)
plt.tight_layout()
plt.savefig('importancia_features_bank.png', dpi=300)
plt.show()

# Análise adicional: curva ROC
# Obter probabilidades para a classe positiva (sim)
...
plt.title('Curva ROC - Dados Bancários')
plt.legend(loc="lower right")
plt.savefig('curva_roc_bank.png', dpi=300)
plt.show()
...
for i, feature in enumerate(top_features):
    plt.subplot(1, 3, i+1)
    try:
        if pd.to_numeric(df[feature], errors='coerce').notna().all():
            sns.histplot(data=df, x=feature, hue='subscribed', bins=10, kde=True)
        else:
    ...
    plt.tight_layout()
plt.savefig('distribuicao_top_features_bank.png', dpi=300)
plt.show()
...
plt.tight_layout()
plt.savefig('matriz_correlacao_bank.png', dpi=300)
plt.show()
